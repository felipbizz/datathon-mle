...
def coluna_valida(df, col):
    ...
    if missing > 0.5:
        print(f'[SKIP] Coluna "{col}" com {missing:.0%} missing, feature não criada.')
        return False
    vc = df[col].value_counts(normalize=True, dropna=True)
    if not vc.empty and vc.iloc[0] > 0.9:
        print(f'[SKIP] Coluna "{col}" com valor dominante ({vc.index[0]}) em {vc.iloc[0]:.0%}, feature não criada.')
        return False
    return True

# precisa melhorar essa lista de palavras chave, isso é só um exemplo pra iniciar o processo
PALAVRAS_CHAVE = ['python', 'gestao', 'sênior', 'senior', 'lideranca', 'liderança', 'java', 'sql', 'analista', 'projeto']
...
if coluna_valida(df, 'data_candidatura') and coluna_valida(df, 'data_status_final'):
    df['tempo_processo_dias'] = (df['data_status_final'] - df['data_candidatura']).dt.days
    print('[OK] Feature tempo_processo_dias criada.')
else:
    print('[SKIP] Feature tempo_processo_dias não criada.')
...
if coluna_valida(df, 'area_atuacao') and coluna_valida(df, 'areas_atuacao'):
    df['match_area_atuacao'] = df.apply(lambda x: int(str(x['area_atuacao']) in str(x['areas_atuacao'])), axis=1)
    print('[OK] Feature match_area_atuacao criada.')
else:
    print('[SKIP] Feature match_area_atuacao não criada.')

# 3. Nível acadêmico do candidato (one-hot encoding)
if coluna_valida(df, 'nivel_academico'):
    df = pd.get_dummies(df, columns=['nivel_academico'], prefix='nivel_acad', dummy_na=True)
    print('[OK] One-hot de nivel_academico criado.')
else:
    print('[SKIP] One-hot de nivel_academico não criado.')

# 4. Tipo de contratação da vaga (one-hot encoding)
if coluna_valida(df, 'tipo_contratacao'):
    df = pd.get_dummies(df, columns=['tipo_contratacao'], prefix='tipo_contr', dummy_na=True)
    print('[OK] One-hot de tipo_contratacao criado.')
else:
    print('[SKIP] One-hot de tipo_contratacao não criado.')
...
if coluna_valida(df, 'data_nascimento'):
    ...
    print('[OK] Feature idade criada.')
else:
    print('[SKIP] Feature idade não criada.')

# 8. Fonte de indicação (binária)
if coluna_valida(df, 'fonte_indicacao'):
    df['indicacao'] = df['fonte_indicacao'].apply(lambda x: int(pd.notnull(x) and x != 'desconhecido'))
    print('[OK] Feature indicacao criada.')
else:
    print('[SKIP] Feature indicacao não criada.')

campos_texto = [
    'cv_pt', 'objetivo_profissional', 'titulo_profissional', 'cursos',
    'titulo_vaga', 'principais_atividades', 'competencia_tecnicas_e_comportamentais',
    'demais_observacoes', 'comentario'
]
for campo in campos_texto:
    if campo in df.columns:
        df[f'{campo}_nchar'] = df[campo].apply(tamanho_texto)
        df[f'{campo}_nwords'] = df[campo].apply(n_palavras)
        print(f'[OK] Features de tamanho para {campo} criadas.')

# Features de palavras-chave
campos_palavras_chave = [
    'cv_pt', 'objetivo_profissional', 'principais_atividades',
    'competencia_tecnicas_e_comportamentais', 'comentario'
]
for campo in campos_palavras_chave:
    if campo in df.columns:
        df[f'{campo}_n_keywords'] = df[campo].apply(conta_palavras_chave)
        print(f'[OK] Feature de palavras-chave para {campo} criada.')

# Feature de quantidade de cursos
if 'cursos' in df.columns:
    df['cursos_n'] = df['cursos'].apply(conta_cursos)
    print('[OK] Feature cursos_n criada.')

# Clusterização de perfis de currículo (cv_pt)
if 'cv_pt' in df.columns:
    print('[INFO] Iniciando clusterização de perfis de currículo...')
    ...
    print(f'[OK] Feature cv_cluster criada com {n_clusters} clusters.')


df.to_parquet('../Datathon Decision/3_silver/dataset_features.parquet', index=False)
print('Dataset com features salvos em ../Datathon Decision/3_silver/dataset_features.parquet')
