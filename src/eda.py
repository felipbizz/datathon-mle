...
sns.countplot(data=prospects, x='situacao_candidado', 
...
print(applicants.describe(include='all', ))

print('\nEstatísticas descritivas - vagas:')
print(vagas.describe(include='all', ))

print('\nEstatísticas descritivas - prospects:')
print(prospects.describe(include='all', ))

# Merge para análise cruzada
df_merged = prospects.merge(applicants[['nome', 'nivel_academico']], on='nome', how='left')

plt.figure(figsize=(10, 5))
sns.countplot(data=df_merged, x='nivel_academico', hue='situacao_candidado', order=df_merged['nivel_academico'].value_counts().index)
...
if 'area_atuacao' in applicants.columns:
    df_merged_area = prospects.merge(applicants[['nome', 'area_atuacao']], on='nome', how='left')
    plt.figure(figsize=(12, 6))
    sns.countplot(data=df_merged_area, x='area_atuacao', hue='situacao_candidado', order=df_merged_area['area_atuacao'].value_counts().index)
    ...
    tab_area = pd.crosstab(df_merged_area['area_atuacao'], df_merged_area['situacao_candidado'])
...
if 'nivel_academico' in df_merged.columns:
    tab_nivel = pd.crosstab(df_merged['nivel_academico'], df_merged['situacao_candidado'])
