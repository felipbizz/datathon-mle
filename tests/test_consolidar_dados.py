...
def test_merge_vagas_prospects(sample_data):
    ...
    # Test merge
    df_merged = pd.merge(df_vagas, df_prospects, on="cod_vaga", how="inner")

    # Check merge results
    assert len(df_merged) == len(
        df_prospects
    )  # Should have same number of rows as prospects
    assert "titulo_vaga" in df_merged.columns  # Should have columns from vagas
    assert (
        "situacao_candidado" in df_merged.columns
    )  # Should have columns from prospects
...
def test_merge_empty_dataframes():
    ...
    df_merged = pd.merge(empty_vagas, empty_prospects, on="cod_vaga", how="inner")
