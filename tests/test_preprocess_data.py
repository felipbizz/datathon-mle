...
def test_clean_data_text_columns():
    ...
    pd.testing.assert_series_equal(result['text_col'], pd.Series(expected, name='text_col'))

def test_clean_data_date_columns():
    # Test data with dates
    df = pd.DataFrame({
        'date_col': ['2023-01-01', '2023-12-31', 'invalid_date', None]
    })
...
def test_clean_data_year_columns():
    ...
    df = pd.DataFrame({
        'year_col': ['2023', '2020', 'invalid', None]
    })
...
def test_clean_data_numeric_columns():
    ...
    df = pd.DataFrame({
        'num_col': ['1,234.56', '2.000,00', 'invalid', None]
    })
