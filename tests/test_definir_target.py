...
def test_target_mapping_consistency():
    ...
    assert len(all_situations) == len(set(all_situations)), "Found duplicate situations across categories"
...
def test_target_categorization():
    ...
    df['target'] = df['situacao_candidado'].apply(lambda x: (
        1 if x in situacao_candidado_aprovado
        else 0 if x in situacao_candidado_nao_aprovado
        else np.nan if x in situacao_candidado_desistente
        else np.nan if x in situacao_candidato_inicial
        else np.nan
    ))
    ...
    pd.testing.assert_series_equal(
        df['target'],
        pd.Series([1.0, 0.0, np.nan, np.nan, np.nan], name='target')
    )
